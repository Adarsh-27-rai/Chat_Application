...
export default function SignUp({ onAuth }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await register({ username: rUser, email: rEmail, password: rPass, displayName: rName });
    ...
    <div className="flex h-screen w-full items-center justify-center bg-gradient-to-br from-[#f0f6ff] to-[#e8f0fe]">
      <div className="w-[420px] bg-white rounded-[24px] shadow-[0_20px_60px_rgba(37,99,235,0.12),0_4px_16px_rgba(0,0,0,0.05)] px-10 pt-10 pb-8 animate-fade-slide">
        ...
        <div className="flex bg-blue-600/5 rounded-xl p-1 mb-7">
          <Link to="/login" className="flex-1 py-[9px] rounded-[10px] bg-transparent text-slate-500 font-medium text-center text-[14px] transition-all duration-200">
            Sign In
          </Link>
        ...
        <form onSubmit={handleSubmit}>
          <Field label="Display Name" value={rName} onChange={(e) => setRName(e.target.value)} placeholder="Your Name" />
          <Field label="Username" value={rUser} onChange={(e) => setRUser(e.target.value)} placeholder="your_username" />
          <Field label="Email" type="email" value={rEmail} onChange={(e) => setREmail(e.target.value)} placeholder="you@example.com" />
          <Field label="Password" type="password" value={rPass} onChange={(e) => setRPass(e.target.value)} placeholder="Min 6 characters" />
